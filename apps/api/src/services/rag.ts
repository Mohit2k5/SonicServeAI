...
export async function retrieveContext(query: string, agentId: string, topK = 5): Promise<string> {
  if (!pc || !process.env.GEMINI_API_KEY) {
    if (!process.env.GEMINI_API_KEY) console.warn('[RAG] Gemini not initialized. Skipping context retrieval.');
    return '';
  }
...
export async function upsertDocuments(docs: { text: string; agentId: string }[]) {
  if (!pc || !process.env.GEMINI_API_KEY) {
  ...
  const index = pc.index(process.env.PINECONE_INDEX!);
  try {
    const vectors = await Promise.all(docs.map(async (doc, i) => {
